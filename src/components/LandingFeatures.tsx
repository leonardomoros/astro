import { Target, FlaskConical, Zap, Plug, Mail, BarChart3 } from 'lucide-react';
...
const ICONS: Record<string, React.ComponentType<{ size?: number; strokeWidth?: number; style?: React.CSSProperties }>> = {
  Target, FlaskConical, Zap, Plug, Mail, BarChart3,
};

const COLORS = ['#3b82f6', '#06b6d4', '#a855f7', '#10b981', '#f59e0b', '#ec4899'];

interface Props { t: LandingPageTranslations['features'] }

export default function LandingFeatures({ t }: Props) {
  return (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {t.items.map((item, i) => {
            ...
            return (
              <div
                key={i}
                className="group relative bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-6 transition-all duration-300 hover:-translate-y-1"
              >
                <div
